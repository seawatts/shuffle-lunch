...
from shuffle.models.shuffle_model import ShuffleModel
...
class ShuffleService:
    def __init__(self, shuffle_file_name, google_api_service):
        ...
        self.__calender_service = CalenderService(google_api_service)
        self.__email_service = EmailService(google_api_service)
    ...
    def __execute_shuffle(self, shuffle):
        try:
            all_accepted = self.__calender_service.get_all_accepted_attendees(shuffle.recurring_event_id, shuffle.calender_group_alias)
            randomized_groups = self.__randomize_service.create_randomize_groups(all_accepted, shuffle.group_size)
            self.__email_service.send_emails_to_groups_with_template(randomized_groups, shuffle.email.fromName, shuffle.email.subject, shuffle.email.template)
